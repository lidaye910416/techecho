...
type PageView = 'main' | 'voice-settings' | 'preferences'
...
export default function Mine() {
  ...
  // --- 语音设置子页 ---
  if (pageView === 'voice-settings') {
    return (
      <View className="mine-page">
        <View className="mine-sub-navbar">
          ...
          <Text className="mine-sub-title">{t('voiceSettings')}</Text>
          <View style={{ width: '60px' }} />
        </View>

        <View className="mine-sub-content">
          <View className="mine-section-header">
            <Text className="mine-section-label">播客风格四声线</Text>
            <Text className="mine-section-hint">选择你喜欢的播报声音</Text>
          </View>

          <View className="mine-voice-grid">
            {VOICES.map((v) => (
              <View
                key={v.id}
    ...
    <View className="mine-page">
      <ScrollView scrollY className="mine-scroll">
          ...
          {user ? (
            <View className="mine-user-card">
              ...
              <View className="mine-logout-btn" onClick={handleLogout}>
                <Text className="mine-logout-text">{t('logout')}</Text>
              </View>
            </View>
          ) : (
            <View className="mine-login-section">
              <View className="mine-login-avatar">
                <Text className="mine-login-emoji">👤</Text>
              </View>
        ...
        <View className="mine-menu-section">
          ...
          <View className="mine-menu-group">
            <View className="mine-menu-item" onClick={() => setPageView('voice-settings')}>
              <Text className="mine-menu-icon">🎙️</Text>
              <View className="mine-menu-info">
                <Text className="mine-menu-text">{t('voiceSettings')}</Text>
                <Text className="mine-menu-sub">
                  {VOICES.find(v => v.id === settings.voice)?.name || '温婉女声'}
                </Text>
              </View>
              <Text className="mine-menu-arrow">›</Text>
            </View>
            ...
            <View className="mine-menu-item" onClick={() => setPageView('preferences')}>
              ...
              <View className="mine-menu-info">
                ...
                <Text className="mine-menu-sub">
                  {t('thresholdTitle')} {settings.threshold} · {settings.darkMode ? '深色' : '浅色'}
                </Text>
              </View>
              <Text className="mine-menu-arrow">›</Text>
            </View>
