...
直接存储 MiniMax 返回的完整音频 URL 到数据库 audio_url 字段。
用户点击「朗读」时直接使用该 URL 播放，避免重复消耗 MiniMax API 配额。
"""
...
async def pre_generate_tts_for_news(
    ...
    """
    预生成 TTS 音频（仅使用 MiniMax）

    注意：
    - 直接存储 MiniMax 返回的完整 URL 到数据库
    - 不保存音频文件到本地（避免静态文件访问问题）
    - 失败时不使用替代服务
    ...
    for i, news in enumerate(news_list):
        ...
        try:
            ...
            stats["success"] += 1
            logger.info(f"TTS pregen [{i+1}/{len(news_list)}] ok {news_id[:24]}... -> {audio_url[:80]}")
